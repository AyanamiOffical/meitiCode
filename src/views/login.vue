...
const onSubmit = async () => {
  formRef.value.validate(async (valid) => {
    ...
    try {
      // Make an HTTP request to the backend for authentication
      const response = await axios.post("https://localhost:8081/user/search", {
      ...
      if (response.data.success) {
        ...
        // Use Element Plus MessageBox to show success message
        ElMessage.success("登录成功！");
        // Delay the navigation for a short time to ensure the message is displayed
        setTimeout(() => {
          // Navigate to the "allin" page
          router.push({ name: "allin" });
        }, 500);
      } else {
        // Authentication failed
        // Use Element Plus MessageBox to show error message
